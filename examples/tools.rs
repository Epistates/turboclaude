//! Example demonstrating tool use with the Anthropic SDK

use anthropic::{
    Client,
    types::{
        MessageRequest, Message, Models, Tool, ToolChoice,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let request = MessageRequest::builder()
        ...
        .messages(vec![
            Message::user("What is 42 multiplied by 17? Also, what's the weather like in Paris?")
        ])
        .build()?;
    ...
    for result in &tool_results {
        if let ContentBlockParam::ToolResult { tool_use_id, content, is_error } = result {
