...
#[cfg(feature = "schema")]
async fn calculator(input: CalculatorInput) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
#[cfg(feature = "schema")]
async fn get_weather(input: WeatherInput) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    // In a real application, you would call a weather API here
    // For this demo, we'll just return mock data
    Ok(format!(
        "Weather in {}: Sunny, 72°F (22°C), Humidity: 45%",
        input.location
    ))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✓ Registered {} tools", runner.tool_count());
    ...
    println!();
    println!("✅ Tool execution loop completed automatically!");
    println!();
    println!("{}", separator);
