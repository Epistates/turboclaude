...
pub struct Response {
    ...
    headers: HeaderMap,
    body: Vec<u8>,
    /// Number of retries taken to get this response
    retries_taken: u32,
...
impl Response {
    ...
    /// Create a new response with timing and retry metadata.
    ...
    /// Get the number of retries taken for this response.
    pub fn retries_taken(&self) -> u32 {
        self.retries_taken
    }

    /// Get the elapsed time for this response.
    ...
    pub fn into_raw<T: DeserializeOwned>(self) -> Result<RawResponse<T>, crate::error::Error> {
        ...
        // Include retry and timing metadata in raw response
    ...
    ///
    /// ```rust,no_run
    /// # use turboclaude::http::{Response, RawResponse};
    /// # use serde::Deserialize;
    /// # #[derive(Deserialize)]
    /// # struct MyType { field: String }
    /// # async fn example(response: Response) -> Result<(), Box<dyn std::error::Error>> {
    /// let raw: RawResponse<MyType> = response.into_parsed_raw()?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn into_parsed_raw<T: DeserializeOwned>(
        self,
    ) -> Result<RawResponse<T>, crate::error::Error> {
        if self.is_error() {
            return Err(crate::error::Error::from_response(
                self.status.as_u16(),
                &self.text()?,
