...
pub type AsyncToolFn<I, O> =
    Arc<dyn Fn(I) -> Pin<Box<dyn Future<Output = O> + Send>> + Send + Sync>;
...
///
/// ```rust,ignore
/// use turboclaude::tools::FunctionTool;
...
mod tests {
    ...
    async fn test_with_name_override() {
        ...
        let tool =
            FunctionTool::new("original_name", "Description", test_func).with_name("custom_name");
    ...
    async fn test_with_description_override() {
        ...
        assert_eq!(
            tool.description(),
            "Custom description that overrides the original"
        );
    }
