...
async fn real_world_streaming_basic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        ttfb.unwrap() < std::time::Duration::from_secs(2),
        "TTFB should be < 2s"
    );
...
async fn real_world_streaming_all_events() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        event_types.contains("ContentBlockStart"),
        "Missing ContentBlockStart"
    );
    assert!(
        event_types.contains("ContentBlockDelta"),
        "Missing ContentBlockDelta"
    );
