...
fn test_client_builder_with_auth_token() {
    let result = Client::builder().auth_token("test-token").build();
...
fn test_client_builder_custom_max_retries() {
    let result = Client::builder().api_key("test-key").max_retries(5).build();
...
fn test_client_builder_custom_headers() {
    let result = Client::builder()
        ...
        .and_then(|builder| builder.default_header("x-another-header", "another-value"))
